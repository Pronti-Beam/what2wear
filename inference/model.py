...
class FullBiLSTM(nn.Module):
    ...
    def forward(self, images, seq_lens, im_lookup_table, txt_lookup_table,
                hidden, texts):
        """Do a forward pass.
        ...
        txt_feats_matrix = torch.nn.functional.normalize(txt_feats_matrix,
                                                         p=2,
                                                         dim=1)

        # Pack the sequences:
        packed_feats = self.create_packed_seq(im_feats, seq_lens,
                                              im_lookup_table)
        # Forward the sequence through the LSTM:
        return packed_feats, (im_feats, txt_feats_matrix), self.lstm(
            packed_feats, hidden)
    ...
    def create_packed_seq(self, feats, seq_lens, im_lookup_table):
        ...
        if feats.is_cuda:
            seqs = autograd.Variable(
                torch.zeros(
                    (len(seq_lens), max(seq_lens), feats.size()[1]))).cuda()
        else:
            seqs = autograd.Variable(
        ...
        for i, seq_len in enumerate(seq_lens):  # Iterate over batch
            for j in range(max(seq_lens)):  # Iterate over sequence
                if j < seq_len:
                    ...
                    seqs[i, j] = autograd.Variable(torch.zeros(
                        feats.size()[1]))

        # In order to be packed, sequences must be ordered from larger to shorter.
        seqs = seqs[sorted(
            range(len(seq_lens)), key=lambda k: seq_lens[k], reverse=True), :]
        ordered_seq_lens = sorted(seq_lens, reverse=True)

        # seqs is (batch size, max length, data_dim)
        if not self.batch_first:
            seqs = seqs.permute(
                1, 0, 2)  # now it is (max length, max length, data_dim)

        return pack_padded_sequence(seqs,
                                    ordered_seq_lens,
                                    batch_first=self.batch_first)
